import { axiosInstance } from "@/lib/axios";
import type { Message, User } from "@/types";
import { create } from "zustand";
import { io } from "socket.io-client";

type ActivityUpdate = {
	userId: string;
	activity: string;
};

interface ChatStore {
	users: User[];
	isLoading: boolean;
	error: string | null;
	socket: any;
	isConnected: boolean;
	onlineUsers: Set<string>;
	userActivities: Map<string, string>;
	messages: Message[];
	messageCache: Record<string, Message[]>;
	selectedUser: User | null;

	fetchUsers: () => Promise<void>;
	initSocket: (userId: string) => void;
	disconnectSocket: () => void;
	sendMessage: (receiverId: string, senderId: string, content: string) => void;
	fetchMessages: (userId: string) => Promise<void>;
	setSelectedUser: (user: User | null) => void;
}

const baseURL = import.meta.env.MODE === "development" ? "http://localhost:5001" : "/";

const socket = io(baseURL, {
	autoConnect: false, // only connect if user is authenticated
	withCredentials: true,
});

export const useChatStore = create<ChatStore>((set, get) => ({
	users: [],
	isLoading: false,
	error: null,
	socket: socket,
	isConnected: false,
	onlineUsers: new Set(),
	userActivities: new Map(),
	messages: [],
	messageCache: {},
	selectedUser: null,

	setSelectedUser: (user) =>
		set((state) => ({
			selectedUser: user,
			messages: user ? state.messageCache[user.clerkId] ?? [] : [],
		})),

	fetchUsers: async () => {
		set({ isLoading: true, error: null });
		try {
			const response = await axiosInstance.get("/users");
			const userMap = new Map<string, User>();
			(response.data as User[]).forEach((user) => {
				userMap.set(user.clerkId, user);
			});
			const uniqueUsers = [...userMap.values()];
			set({ users: uniqueUsers });
		} catch (error: any) {
			set({ error: error.response.data.message });
		} finally {
			set({ isLoading: false });
		}
	},

	initSocket: (userId) => {
		if (!get().isConnected) {
			socket.removeAllListeners();
			socket.auth = { userId };
			socket.connect();

			socket.emit("user_connected", userId);

			socket.on("users_online", (users: string[]) => {
				set({ onlineUsers: new Set(users) });
			});

			socket.on("activities", (activities: [string, string][]) => {
				set({ userActivities: new Map(activities) });
			});

			socket.on("user_connected", (userId: string) => {
				set((state) => ({
					onlineUsers: new Set([...state.onlineUsers, userId]),
				}));
			});

			socket.on("user_disconnected", (userId: string) => {
				set((state) => {
					const newOnlineUsers = new Set(state.onlineUsers);
					newOnlineUsers.delete(userId);
					return { onlineUsers: newOnlineUsers };
				});
			});

			socket.on("receive_message", (message: Message) => {
				set((state) => {
					const activeUserId = state.selectedUser?.clerkId;
					const otherUserId = message.senderId;
					const cachedMessages = otherUserId ? state.messageCache[otherUserId] ?? [] : [];
					const updatedMessages = [...cachedMessages, message];

					return {
						messageCache: otherUserId
							? { ...state.messageCache, [otherUserId]: updatedMessages }
							: state.messageCache,
						messages: otherUserId && activeUserId === otherUserId ? updatedMessages : state.messages,
					};
				});
			});

			socket.on("message_sent", (message: Message) => {
				set((state) => {
					const activeUserId = state.selectedUser?.clerkId;
					const otherUserId = message.receiverId;
					const cachedMessages = otherUserId ? state.messageCache[otherUserId] ?? [] : [];
					const updatedMessages = [...cachedMessages, message];

					return {
						messageCache: otherUserId
							? { ...state.messageCache, [otherUserId]: updatedMessages }
							: state.messageCache,
						messages: otherUserId && activeUserId === otherUserId ? updatedMessages : state.messages,
					};
				});
			});

			socket.on("activity_updated", ({ userId, activity }: ActivityUpdate) => {
				set((state) => {
					const newActivities = new Map(state.userActivities);
					newActivities.set(userId, activity);
					return { userActivities: newActivities };
				});
			});

			set({ isConnected: true });
		}
	},

	disconnectSocket: () => {
		if (get().isConnected) {
			socket.disconnect();
			socket.removeAllListeners();
			set({
				isConnected: false,
				onlineUsers: new Set(),
				userActivities: new Map(),
				messages: [],
				messageCache: {},
			});
		}
	},

	sendMessage: async (receiverId, senderId, content) => {
		const socket = get().socket;
		if (!socket) return;

		socket.emit("send_message", { receiverId, senderId, content });
	},

	fetchMessages: async (userId: string) => {
		const cachedMessages = get().messageCache[userId] ?? [];
		set({ isLoading: true, error: null, messages: cachedMessages });
		try {
			const response = await axiosInstance.get(`/users/messages/${userId}`);
			set((state) => ({
				messages: response.data,
				messageCache: {
					...state.messageCache,
					[userId]: response.data,
				},
			}));
		} catch (error: any) {
			set({ error: error.response.data.message });
		} finally {
			set({ isLoading: false });
		}
	},
}));
